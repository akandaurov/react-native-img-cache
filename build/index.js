...
const getTempFileName = () => BASE_DIR + "/" + Math.random().toString(36).substr(2, 9);
export class ImageCache {
    ...
    download(cache) {
        ...
        if (!cache.downloading) {
            ...
            cache.task = RNFetchBlob.config({ path: tempFileName }).fetch(method, uri, source.headers);
            cache.task.then(() => {
                RNFetchBlob.fs.mv(tempFileName, path).then(() => {
